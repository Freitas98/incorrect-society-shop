...
- Modelo 3D e dependências pesadas só carregam depois de escolher uma fonte. Three.js 0.180.0 e MediaPipe Tasks Vision 0.10.22-rc.20250304 vendorizados em `assets/vto-*`; WASM, Pose Full e Selfie Multiclass 256 são obtidos de URLs fixas Google/jsDelivr. Licenças e reprodução em `docs/TRY-ON.md` e `scripts/vendor-try-on.cjs`.
...
- GLB principal e LOD leve preservam estampados e esqueleto. Memória/CPU indicadas pelo browser selecionam o modo automático; o seletor manual permite modo leve mantendo peça, medidas e fonte. DPR, frequência de inferência e iterações do tecido são limitados. Medir em condições comparáveis; emulação com CPU limitada não equivale ao hardware de um telemóvel.
- Testes: `node --test tests/*.test.cjs` (58 passaram; Liquid requer dependência isolada documentada). Scripts locais de integração, comércio simulado, resiliência, oclusão, vídeo e performance em `scripts/vto-*.cjs`; nunca usam câmara real, carrinho real ou uploads. Fotografias, vídeo público e a referência do utilizador foram testados. Depois da publicação, ambos os produtos ajustaram uma fotografia usando os recursos reais do CDN Shopify, num HTML/catálogo de teste local, sem erros JS/worker. A montra continua protegida por password: a publicação/entrega dos recursos está verificada, mas as páginas de produto autenticadas, câmara física iPhone/Android e fidelidade comparada com as peças vestidas aguardam teste do utilizador. Não confundir esses níveis de validação.
