...
(async()=>{
  ...
  try{
    ...
    const angles=report.fits.map(f=>f.angle),unwrapped=[angles[0]];
    for(let i=1;i<angles.length;i++)unwrapped.push(unwrapped[i-1]+Math.atan2(Math.sin(angles[i]-angles[i-1]),Math.cos(angles[i]-angles[i-1])));
    const range=Math.max(...unwrapped)-Math.min(...unwrapped);
